...
namespace FreeLauncher.Forms
{
    public partial class LauncherForm : RadForm
    {
        ...
        public LauncherForm(Configuration configuration)
        {
            ...
            AppendLog($"Idioma cargado: {_configuration.Localization.Name}({_configuration.Localization.LanguageTag})");
            AppendLog(new string('=', 12));
            AppendLog("Informacion del Systema :");
            AppendLog($"{new string(' ', 2)}Systema operativo:");
...
<h1>Modo sin conexion Activado</h1>
<hr />
No se pudo conectar a internet o el modo sin conexion fue activado.
<br />
Porfavor comprueba tu conexion a internet y reinicia el Launcher.
<hr />
<i>{ProductName} {ProductVersion}</i>
</body></html>";
                Text += " [OFFLINE]";
            } else {
                newsBrowser.Navigate("https://anycraft-server.blogspot.com/");
            ...
            bgw.RunWorkerCompleted += delegate {
                ...
                bgw1.RunWorkerCompleted += delegate {
                    ...
                    bgw2.RunWorkerCompleted += delegate {
                        ...
                        AppendLog($"Version {selectedVersionManifest.VersionId} correctamente iniciada.");
                        SetControlBlockState(false);
                        UpdateVersionListView();
                        _versionToLaunch = null;
                    };
        ...
        private void ProcessUrl()
        {
            ...
            if (newsBrowser.Url != new Uri("https://anycraft-server.blogspot.com/")) {
        ...
        private void versionsListView_ItemMouseClick(object sender, ListViewItemEventArgs e)
        {
            ...
            delVerButton.Click += delegate {
                ...
                try {
                    ...
                    AppendLog($"Version '{versionsListView.SelectedItem[0]}' fue borrada exitosamente.");
                    UpdateVersionListView();
                } catch (Exception ex) {
                    AppendException($"Ocurrio un error mientras se borraba la version: {ex}");
                }
            };
        ...
        private void UpdateVersions()
        {
            ...
            if (_configuration.Arguments.OfflineMode) {
                AppendLog("No fue posible obtener las versiones: modo sin conexion activado.");
            ...
            AppendLog("Comprobando version.json...");
            ...
            AppendLog("Ultima snapshot: " + remoteManifest.LatestVersions.Snapshot);
            AppendLog("Ultima release: " + remoteManifest.LatestVersions.Release);
            RawVersionListManifest localManifest =
                RawVersionListManifest.ParseList(File.ReadAllText(versionsManifestPath));
            AppendLog($"Versiones locales: {localManifest.Versions.Count}. "
                + $"Versiones remotas: {remoteManifest.Versions.Count}");
            if (remoteManifest.Versions.Count == localManifest.Versions.Count &&
                ...
                AppendLog("No se encontro actualizacion.");
                return;
            }
            AppendLog("Escribiendo nueva lista de versiones...");
            File.WriteAllText(versionsManifestPath, remoteManifest.ToString());
            _versionList = remoteManifest;
        }
        ...
        private void UpdateProfileList()
        {
            ...
            try {
                ...
                if (!_profileManager.Profiles.Any()) {
                    throw new FileLoadException("El archivo esta corrupto o no contiene perfiles.");
                }
            } catch (Exception ex) {
                AppendException($"Una excepcion a ocurrido mientras se procesaban los perfiles:\n{ex.Message}\nUna nueva lista de perfiles a sido creada.");
                if (File.Exists(profilesPath)) {
                    string fileName = $"launcher_profiles-{LinuxTimeStamp}.bak.json";
                    AppendLog("Una copio de los perfiles viejos fue creada: " + fileName);
        ...
        private void DownloadVersion(string version)
        {
            ...
            AppendLog($"Comprobando disponibilidad de la version '{version}' ...");
            ...
            if (!File.Exists($@"{path}\{version}.json") || _restoreVersion) {
                if (!_configuration.Arguments.OfflineMode) {
                    ...
                    UpdateStatusBarAndLog($"Descargando {filename}...", new StackFrame().GetMethod().Name);
                    ...
                    AppendException($"No se pudo descargar la version {version}: modo sin conexion activado.");
                    return;
                }
            }
            StatusBarValue = 0;
            ...
            AppendLog($@"Comprobacion finalizada para la disponibilidad de la version {version}.");
        }
        ...
        private string DownloadLibraries()
        {
            ...
            AppendLog("Preparando librearias requeridas...");
            ...
            foreach (DownloadEntry entry in libsToDownload.Keys) {
                ...
                if (!File.Exists(_configuration.McLibs + @"\" + entry.Path) ||
                    _restoreVersion) {
                    if (!_configuration.Arguments.OfflineMode) {
                        UpdateStatusBarAndLog($"Descargando {entry.Path.Replace('/', '\\')}...");
                        ...
                        try {
                            ...
                            AppendException("Descarga Fallida: " + ex.Message);
                        ...
                        AppendException($"No se pudo descargar {entry.Path}: modo sin conexion activado.");
                    }
                }
                if (entry.IsNative) {
            ...
            AppendLog("Comprobacion de librearias finalizado.");
            return libraries.ToString();
        }

        private void DownloadAssets()
        {
            UpdateStatusBarAndLog("Comprobando datos del juego...");
            VersionManifest selectedVersionManifest = VersionManifest.ParseVersion(
                new DirectoryInfo(_configuration.McVersions + @"\" +
                    (_versionToLaunch ??
                        (_selectedProfile.SelectedVersion ?? GetLatestVersion(_selectedProfile)))));
            ...
            StatusBarValue = 0;
            UpdateStatusBarAndLog("Descargando datos del juego...");
            ...
            foreach (Asset asset in manifest.Objects.Select(pair => pair.Value).Where(asset => !File.Exists(_configuration.McDirectory + @"\assets\objects\" +
                ...
                try {
                    AppendDebug($"Descargando {asset.Hash}...");
            ...
            AppendLog("Comprobacion de datos del juego terminada.");
        ...
        public void Launch()
        {
            if (_profile.LauncherVisibilityOnGameClose != Profile.LauncherVisibility.CLOSED) {
                ...
                if (_launcherForm.EnableMinecraftLogging.Checked) {
                    ...
                    AppendLog($"NOTICE:{Environment.NewLine}Logs del juego han sido desactivados. Se pueden Activar en Otros y ajustes.{Environment.NewLine}{Environment.NewLine}");
                }
